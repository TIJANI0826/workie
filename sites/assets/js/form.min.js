(function () {
	...
	frappe.ui.form.ControlCode = frappe.ui.form.ControlText.extend({
		...
		make_ace_editor: function make_ace_editor() {
			...
			this.expanded = false;
			this.$expand_button = $(("<button class=\"btn btn-xs btn-default\">" + (__('Expand')) + "</button>")).click(function () {
		...
		toggle_label: function toggle_label() {
			var button_label = this.expanded ? __('Collapse') : __('Expand');
	...
	// ignored /home/ibrahim/frappe-bench/apps/frappe/frappe/public/js/frappe/form/controls/quill-mention/quill.mention.css
			...
			docinfo.versions.forEach(function(version) {
				...
				if (data.row_changed && data.row_changed.length) {
					...
					if (parts$1.length) {
						var message$1;
						if (updater_reference_link) {
							message$1 = __("changed values for {0} {1}", [parts$1.join(', '), updater_reference_link]);
						} else {
							message$1 = __("changed values for {0}", [parts$1.join(', ')]);
						}
						out.push(me.get_version_comment(version, message$1));
					}
				}
	...
	frappe.ui.form.Form = class FrappeForm {
		...
		show_tour(on_finish) {
			...
			var steps = frappe.tour[this.doctype].map(function (step) {
